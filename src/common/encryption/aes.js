...
const encrypt = (obj) => {
    ...
    return Buffer.from(encryptedBytes)
        ...
        .replace(/\+/g, plusReplacement);
};

const decrypt = (cypher) => {
    // parse the cypher into bytes
    const slashReplacementPattern = new RegExp(slashReplacement, 'g');
    const plusReplacementPattern = new RegExp(plusReplacement, 'g');
    const encryptedBuffer = Buffer.from(
        cypher
            .replace(slashReplacementPattern, '/')
            .replace(plusReplacementPattern, '+')
        , 'base64'
    );
